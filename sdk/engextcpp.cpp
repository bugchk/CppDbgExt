...
void WINAPI
ExtCurrentThreadHolder::Set(ULONG64 pThread)
{
	HRESULT Status;

	if ((Status = g_Ext->m_System2->
		SetImplicitThreadDataOffset(pThread)) != S_OK)
	{
		throw ExtStatusException(Status,
			"ExtCurrentThreadHolder::Set failed");
	}
}

ULONG64 WINAPI
ExtCurrentThreadHolder::Get()
{
	HRESULT Status;
	ULONG64 pThread;

	if ((Status = g_Ext->m_System2->
		GetImplicitThreadDataOffset(&pThread)) != S_OK)
	{
		throw ExtStatusException(Status,
			"ExtCurrentThreadHolder::Get failed");
	}
...
void WINAPIV
ExtExtension::OutLine(_In_ PCSTR Format,
	...)
{
	va_list Args;
...
void WINAPIV
ExtExtension::Status(_In_ PCSTR Format,
	...)
{
	va_list Args;

	va_start(Args, Format);
	m_Control->OutputVaList(DEBUG_OUTPUT_STATUS, Format, Args);
	va_end(Args);
}
...
void WINAPIV
ExtExtension::OutLine(_In_ PCWSTR Format,
	...)
{
	va_list Args;
	...
	va_start(Args, Format);
	m_Control4->OutputVaListWide(m_OutMask, Format, Args);
	m_Control4->OutputWide(m_OutMask, L"\n");
	va_end(Args);
}
...
void WINAPIV
ExtExtension::Status(_In_ PCWSTR Format,
	...)
{
	va_list Args;

	va_start(Args, Format);
	m_Control4->OutputVaListWide(DEBUG_OUTPUT_STATUS, Format, Args);
	va_end(Args);
}
...
void WINAPIV
ExtExtension::OutTimeString(ULONG64 mSec)
{
	...
	Out("(%01d:%02d:%02d:%02d.%03d)", days, hours, minutes, seconds, ms);
}
...
ULONG64 
ExtExtension::GetUlong64(_In_ PCSTR Str) throw(...)
{
	ULONG64 buffer; ULONG read;
	ULONG64 offset = EvalExprU64(Str);
	HRESULT hr = g_Ext->m_Data->ReadVirtual(offset, (BYTE*)&buffer, 8, &read);
	if (hr != S_OK)
	{
		g_Ext->ThrowRemote(hr, "Unable to read %s at %p", Str, offset);
	}
	return buffer;
}
LONG64 
ExtExtension::GetLong64(_In_ PCSTR Str) throw(...)
{
	LONG64 buffer; ULONG read;
	ULONG64 offset = EvalExprU64(Str);
	HRESULT hr = g_Ext->m_Data->ReadVirtual(offset, (BYTE*)&buffer, 8, &read);
	if (hr != S_OK)
	{
...
ULONG 
ExtExtension::GetUlong(_In_ ULONG64 Addr) throw(...)
{
	ULONG buffer; ULONG read;
	HRESULT hr = g_Ext->m_Data->ReadVirtual(Addr, (BYTE*)&buffer, 4, &read);
	if (hr != S_OK)
	{
		g_Ext->ThrowRemote(hr, "Unable to read %p", Addr);
	}
	return buffer;
}
LONG 
ExtExtension::GetLong(_In_ ULONG64 Addr) throw(...)
{
	LONG buffer; ULONG read;
	HRESULT hr = g_Ext->m_Data->ReadVirtual(Addr, (BYTE*)&buffer, 4, &read);
	if (hr != S_OK)
	{
		g_Ext->ThrowRemote(hr, "Unable to read %p", Addr);
...
ULONG64 
ExtExtension::GetUlong64(_In_ ULONG64 Addr) throw(...)
{
	ULONG64 buffer; ULONG read;
	HRESULT hr = g_Ext->m_Data->ReadVirtual(Addr, (BYTE*)&buffer, 8, &read);
	if (hr != S_OK)
	{
		g_Ext->ThrowRemote(hr, "Unable to read %p", Addr);
	}
...
LONG64 
ExtExtension::GetLong64(_In_ ULONG64 Addr) throw(...)
{
	LONG64 buffer; ULONG read;
	HRESULT hr = g_Ext->m_Data->ReadVirtual(Addr, (BYTE*)&buffer, 8, &read);
	if (hr != S_OK)
...
ExtNtOsInformation::GetKernelLoadedModule(_In_ ULONG64 Offset)
{
    ...
    return ExtRemoteTyped("nt!_LDR_DATA_TABLE_ENTRY",
                          Offset,
                          true,
                          &s_KernelLoadedModuleBaseInfoCookie,
                          "InLoadOrderLinks");
